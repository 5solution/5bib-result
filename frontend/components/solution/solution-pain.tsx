...
export function SolutionSocialProof({ lang }: { lang: Lang }) {
  ...
  return (
    <section style={{ maxWidth: 1280, margin: '0 auto', padding: '64px 32px 16px' }}>
      ...
      <div className="solution-marquee-wrap">
        <div className="solution-marquee-track">
          {[...clients, ...clients].map((c, i) => (
            <div key={i} className="solution-marquee-item">
              <span className="solution-marquee-name">{c.n}</span>
              <span className="solution-marquee-sub">{c.s}</span>
              <span className="solution-marquee-sep">✦</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <section style={{ maxWidth: 1280, margin: '0 auto', padding: '80px 32px 20px' }} id="product">
      <div style={{ marginBottom: 50, maxWidth: 820 }}>
        ...
        <h2 className="type-campaign" style={{ fontSize: 'clamp(36px, 4.4vw, 64px)', color: 'var(--5s-text)', margin: '10px 0 0' }}>
          ...
          <MagneticText
            text={t('cả 3 đều đau.', 'all three hurt.')}
            hoverText={t('5BIB fix luôn.', '5BIB fixes it.')}
            textClassName="solution-pain-magnetic-text"
            hoverTextClassName="solution-pain-magnetic-reveal"
            circleClassName="solution-pain-magnetic-circle"
            circleSize={220}
          />
        </h2>
