    ...
    <Card className="border-stone-200">
      ...
      <CardContent>
        {podium.athletes.length === 0 ? (
          <div className="text-sm text-stone-500">Chưa có athlete trong AG này.</div>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-stone-100">
              {podium.athletes.map((a) => (
                <tr key={a.bib} className={a.rank <= 3 ? 'bg-amber-50 font-semibold' : ''}>
                  <td className="px-2 py-1 font-bold">
                    {a.rank === 1 ? '1st' : a.rank === 2 ? '2nd' : a.rank === 3 ? '3rd' : `#${a.rank}`}
                    {a.tied && ' *'}
                  </td>
                  ...
                  {expanded && <td className="px-2 py-1">{a.ageOnRaceDay ?? '—'}</td>}
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {expanded && (
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <div className="mb-1 text-xs font-semibold text-stone-700">Lịch sử trạng thái</div>
              <StateMachineTimeline history={podium.stateHistory} />
            </div>
            <div>
              <div className="mb-1 text-xs font-semibold text-stone-700">Hành động BTC</div>
