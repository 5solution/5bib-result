...
/**
 * F-009 CourseMapFullView — VERBATIM PORT of F-006 CourseMapTabInner (BR-AF-23 7th port).
 *
 * Single delta vs source: filename + component/interface name + this docblock.
 * All Leaflet logic preserved byte-for-byte from F-006:
 *   - escapeHtml XSS guard (TD-F006-07)
 *   - FitBounds sovereignty safeguard (BR-CM-13)
 *   - estimatePlaceholderPosition catch-22 fallback
 *   - snap-to-polyline 50m
 *   - manual-mode draggable + key={cp.key}-${manualMode} re-mount
 *   - GeoJSON inline-vs-S3-fetch fallback (BR-CM-11b)
 *
 * Imported via `next/dynamic({ ssr: false })` from CourseMapLayout so SSR
 * doesn't choke on Leaflet's `window` references.
 */
...
interface CourseMapFullViewProps {
  raceId: string;
  courseId: string;
...
export default function CourseMapFullView({
  raceId,
  courseId,
  gpxSimplifiedUrl,
  geoJson,
  ...
  manualMode,
  unmatchedKeys,
}: CourseMapFullViewProps): React.ReactElement {
