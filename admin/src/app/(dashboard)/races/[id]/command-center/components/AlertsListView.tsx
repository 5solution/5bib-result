...
/**
 * F-008 v2 BR-CC2-35 — AlertsListView (verbatim port of F-005 AlertsTab.tsx).
 *
 * Single import-path fix only (BR-AF-23 precedent): import `AlertDetailDialog`
 * via the local wrapper instead of reaching back into `timing-alerts/`.
 * All other logic byte-for-byte identical to the F-005 source.
 *
 * Drill-in target for `/command-center?view=alerts` query param (BR-CC2-32 +
 * BR-CC2-36). Mounted by CommandCenterLayout when `view === 'alerts'`.
 */
...
import { AlertDetailDialog } from './AlertDetailDialogWrapper';
...
export function AlertsListView({ raceId }: { raceId: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {(['OPEN', 'RESOLVED', 'FALSE_ALARM'] as TimingAlertStatus[]).map(
          (s) => {
            const count = s === 'OPEN' ? (stats?.open_count ?? 0) : null;
            return (
              <Button
                key={s}
      ...
      {isInitialLoading ? (
        ...
        SEVERITY_ORDER.map((sev) => {
          ...
          const totalLoadedSev =
            q.data?.pages.flatMap((p) => p.items).length ?? 0;
          const truncated = list.length < totalLoadedSev;
          return (
            <Card key={sev}>
              <CardHeader>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-[280px] flex-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {alert.detection_type === 'CUTOFF_RISK' && (
              <Badge
                variant="outline"
                className="border-amber-400 bg-amber-50 text-amber-800"
                title="F-010 — Projected finish vượt cutoff time. Auto-resolves khi athlete qua CP tiếp theo hoặc finish."
              >
                CUTOFF RISK
              </Badge>
            )}
          ...
          <div className="mt-1 text-sm text-stone-700">
            ...
            {alert.last_seen_time}) → Miss{' '}
            <strong>{alert.missing_point}</strong>
          </div>
