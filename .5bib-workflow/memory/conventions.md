...
| Override shadcn primitive class trong `className` prop bằng class KHÔNG có responsive variant prefix → KHÔNG override class có `sm:`/`md:`/`lg:` variant trong default | shadcn admin `DialogContent` có `sm:max-w-sm` mặc định. Pass `max-w-6xl` qua className → tailwind-merge thấy 2 class khác variant scope (base vs `sm:`) → KHÔNG conflict → ở screen ≥640px `sm:max-w-sm` (384px) thắng → dialog vẫn hẹp dù mày tưởng đã override | DÙNG matching variant prefix `sm:max-w-6xl` HOẶC `!important` modifier `!max-w-6xl sm:!max-w-6xl`. Áp dụng cho mọi prop conflict: width, padding, gap, height. v2 hotfix `9c6df03` 2026-05-14 sau Danny screenshot v1 hotfix `6c6ce8a` vẫn hỏng. Lesson: kiểm `components/ui/[primitive].tsx` ĐỌC default className thấy responsive variant nào → match prefix khi override. |
| `<SelectValue />` empty children với `@base-ui/react/select` Base UI v1.x (admin lib) | Khác Radix `@radix-ui/react-select` — Base UI Select.Value KHÔNG auto-lookup matched `<SelectItem>` children theo value. Mặc định render raw enum string (`TIMING`, `CONTRACT`, `DRAFT`...). Vi phạm Display Convention F-028 (`f18da46`). | DÙNG render prop pattern: `<SelectValue>{(v) => LABEL_MAP[v] ?? v}</SelectValue>` với centralized Record dictionary. F-024 wizard hotfix `e166970` 2026-05-14. 55+ instances khác trong admin có TD-ADMIN-SELECTVALUE-SWEEP. Khi tạo dropdown mới: KHÔNG copy-paste `<SelectValue />` từ shadcn docs (Radix-era); LUÔN check primitive lib + dùng render prop. |
| Picker/Selector với always-visible list không collapse sau khi user select | Vertical space lost — list 10-20 items + form auto-fill đè nhau, user phải scroll qua list để thấy form. Bug F-024 PartnerPicker (Danny screenshot 2026-05-14). | Pattern UX-PICKER-COLLAPSE: thêm `browsing` state. Conditional render: `value && !browsing` → collapsed compact card (label + key fields + "Đổi" button); else → search input + list. Auto-collapse sau pick / sau tạo mới. Hotfix `e166970` PartnerPicker. Áp dụng cho mọi picker: PartnerPicker, ProviderPicker, RacePicker, ServiceCatalogPicker, TenantPicker. |
| Đọc field qua `(x as any).field_not_in_schema` cast | Field "lậu" không declared trong schema → invisible cho Coder/QC review. Bug recon docx `tenant_metadata` field cast → masked merchant info bug pre-existing từ F-001 đến 2026-05-14. | Hoặc declare field trong schema (preferred), hoặc dùng explicit interface `interface ReconciliationWithMetadata extends Reconciliation { tenant_metadata?: TenantMetadata }`. Khi service đọc field qua any-cast → audit schema declare có field đó không. Nếu không → flag risk + add comment giải thích tại sao dynamic. Bug-fix `b218a53` docx merchant source. |
| Multi-source data field (platform sync + admin override + schema fallback) — KHÔNG document priority chain | Service đọc 1 source ngẫu nhiên (thường là source dễ access nhất), bỏ qua source ưu tiên. Bug docx merchant info đọc `tenant_metadata` (platform readonly) thay vì `MerchantConfig` admin-entered (single source of truth pháp lý). | Pattern: "Admin-entered > Platform sync > Schema fallback" — explicit priority chain documented trong service code comment block + audit log per call. Áp dụng cho mọi docs/exports/forms consume merchant/tenant/partner info. Bug-fix `b218a53` recon docx (5 NEW TC-MC-* tests). |
| Hand-pick field mapping trong service create/update/init — quên field mới khi schema/DTO update | Field "lậu" giữa schema (mongoose) + DTO (class-validator) + service mapping (transform). Save thành công nhưng field bị drop silent. Reload thấy empty → user nghĩ "không lưu được". Bug F-035 cost field: backend `contracts.service.create+update` quên `cost: li.cost ?? 0`, admin `buildInitialState` cũng quên. Triple-quên trên cả backend + admin = 1 ngày bug chase. | Pattern: khi thêm field mới vào schema, **GREP tất cả `.map((li) =>` hoặc `.lineItems.map`** trong service + admin transform layer + spec → audit từng map. Hoặc dùng spread `{...li, computed_fields_only}` thay vì hand-pick (less risky). Bug-fix `797fa85` backend + `7bc1050` admin. |
| "Actual overrides Estimated" semantic mà không hỏi business intent | Cost source attribution: "actual" với "estimated" có 2 cách hiểu — REPLACE (actual thay thế estimate khi có) hay INCREMENTAL (actual ADD-ON estimate). Bug F-033 designed REPLACE → khi admin nhập 1 chi phí phát sinh 1M, P&L hiện 1M thay vì 185M+1M=186M → margin 99.5% thay vì 11%. | Cost flows trong business reality thường là ADDITIVE: line items.cost = quote-time base estimate, cost_items = chi phí phát sinh THÊM. Manager `/5bib-plan` checklist add: "Multi-source cost/revenue → hỏi business REPLACE hay INCREMENTAL trước khi design priority chain." Bug-fix F-036 `a8ad737` switch sang additive với 6 TC-LIC tests. |
| Push commit thẳng `release/v*` branch sau mỗi hotfix → CI auto-deploy PROD mỗi commit | "Cowboy ops" — user bị làm guinea pig trên PROD. Không có DEV staging UAT. Bug fix có thể introduce regression khác → mỗi loop = 1 PROD rotate. Hôm 2026-05-14 tao ship 11 commits liên tiếp lên release/v1.8.1 mà Danny phải catch bug từng cái trên PROD. | Workflow chuẩn: feature branch off main → push branch only (CI build, NO deploy) → merge main → DEV deploy (`admin-dev.5bib.com`) → user UAT trên DEV → cherry-pick/merge to `release/v*` → PROD deploy. Push thẳng `release/v*` CHỈ cho critical incident (security, data corruption, complete PROD outage). Memory hardened sau Danny instruction "tạo nhánh ra để fix chứ đừng golive liên tục như này nữa". Branch `fix/F-035-edit-dialog-line-items-width` là first example đúng workflow trong session. |
