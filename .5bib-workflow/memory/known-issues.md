...
| **TD-F083-SECTIONFORMS** 🟢 LOW | admin / LandingBuilder | section `data` edit bằng JSON textarea (MVP), KHÔNG per-type visual forms. Section list có controls đủ (enable/▲▼/variant). Deviation D3. | 10 bespoke per-type forms = separate large build. | Phase 2: rich repeater forms (hero CTA, pricing tiers, schedule items). |
...
| **TD-F083-C2-ADMIN-AUTH-WALKTHROUGH-PRE-PROD** 🟡 MED ⏳ | landing / admin UI | C2 admin-auth UI walkthrough (create→configure→publish qua giao diện admin với Logto admin login) CHƯA verify live — env QC không có Logto admin token. Logic unit-tested (15 TC) + guard 401-verified + public luồng live E2E green. | QC env thiếu Logto admin login; seed-direct-Mongo đã prove public flow. | **Pre-PROD:** Danny/Manager login admin thật → create landing → set subdomain → publish → verify trang lên. Trước khi cut release/* PROD. |
