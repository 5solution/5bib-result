...
|----|--------|------|-----------|------------------------------|
| **TD-F062-MOM-BOUNDARY-ROLLOVER** 🟡 MED 🔴 BLOCKING Wave 2 | analytics / period-resolver | `resolveCompare('mom')` dùng `setUTCMonth(-1)` rolls over khi source day > target month days. Verified bug: `2026-05-31 setUTCMonth(3)` (April) → JS rolls to `2026-05-01` instead of `2026-04-30`. Worst-case: marketing team picks "Tháng 5 (1-31)" period → MoM toggle → "kỳ trước" shows May 1-30 (current month) instead of April 30 - May 30 → MoM growth = 0% wrong report. | Latent Wave 1 (no consumer của 'mom' yet). Manager spot-check 2026-05-22 caught (Coder + QC both missed). | **Wave 2 Coder MUST fix FIRST trước khi backend services**: replace `setUTCMonth` với `shiftMonthClamped(date, months)` pattern (clamp day to last-day-of-target-month) per `MANAGER_WAVE1_REVIEW.md` code suggestion. Add boundary test `it('mom: handles May 31 → April 30 WITHOUT rollover')` + day=29/30 leap-year edge cases. ~30 min fix. |
| **TD-F062-VALIDATION-COMPAREKIND** 🟢 LOW | analytics / controller | `analytics.controller.ts` 4 sites cast `q.compareWith as CompareKind` accept any string từ query param. Invalid values fall through to switch default branch (yoy). | QC adversarial probe Wave 1 finding. | Wave 2 — add `@IsEnum(['prev','yoy','custom','none','wow','mom'])` decorator vào `AnalyticsQueryDto.compareWith` để class-validator reject invalid input at DTO level (400 vs silent fallback). ~5 LoC fix. |
| **TD-F062-F026-SILENT-CAPABILITY-EXPANSION** 🟢 INFORMATIONAL | analytics / 6 F-026 endpoints | Adj #1 CompareKind extend silently adds `wow/mom` capability cho 6 F-026 endpoint hiện tại (repeat-athlete-rate, merchant-churn, time-to-fill, claim-rate, geographic-demographic, refund-cancel-rate). Cache key namespace separate (no collision). Pure capability gift, no degradation. | QC Wave 1 finding. | Wave 5 decision: market as feature OR add explicit guard restrict F-026 chỉ accept original 4 values `prev/yoy/custom/none`. Recommend market as feature (free UX upgrade). |
