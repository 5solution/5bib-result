...
└── modules/
    ...
    ├── promo-hub/              # ⭐ FEATURE-027 NEW: Configurable Landing Page main module. 1 schema (`promo-hubs` w/ sections subdoc array 19 type enum: hero, race_calendar, featured_races, promo_banner, cta_buttons, sponsors, stats, rich_text, recent_results, link_grid, social_links, faq, countdown, video_embed, image_gallery, testimonial, map_embed, schedule_timeline, form_embed) + 5 DTOs + service + controller + 2 spec files (5 controller TC + 17 service TC). 7 endpoints: list, create, findById, findBySlug (PUBLIC, rate-limit by IP), update, reorderSections, delete. ANTI-STAMPEDE SETNX lock pattern port từ F-004 RaceMasterDataService (`promo-hub-lock:<slug>` TTL 5s + retry 3×200ms). sanitize-html backend strip <script>/event handlers/javascript: URIs trên rich_text + customCss write path. RBAC LogtoAdminGuard cho admin endpoints. Section-as-subdoc array pattern (read-heavy, atomic save). ⭐ FEATURE-033: NEW entity `RaceReadonly` (MySQL platform DB `5bib_platform_live` named connection `'platform'`) + `findRacesOnSale()` + `GET /api/promo-hubs/races-on-sale` (PUBLIC). ⭐ FEATURE-037 V2 (2026-05-18 on-sale-race-detail-page): extended `RaceReadonly` +8 cols (description/images/eventType/raceType/district/season/locationUrl/province) + NEW entity `OnSaleCourseReadonly` (table `race_course` 16 cols — renamed from initial `RaceCourseReadonly` to avoid TS identifier collision with existing race-master-data/RaceCourseReadonly 3-col entity, TypeORM multi-entity-per-table via different class names) + NEW DTOs `RaceOnSaleDetailDto` (22 fields) + `RaceCourseDto` (17 fields) + `findRaceOnSaleByUrlName(urlName)` service (~70 lines, OR `url_name=:urlName OR race_id=:raceId` with `^\d+$` numeric regex safe parse, parameterized TypeORM, bit field CAST pattern) + NEW endpoint `GET /api/promo-hubs/races-on-sale/by-url-name/:urlName` (PUBLIC, no auth same F-033 pattern). Cache `promo-hub:race-on-sale-detail:<urlName>` TTL 600s, TTL-only invalidation per BR-37-19. Module wiring BOTH `forFeature` + `forRoot(entities[])` per F-033 critical lesson. Resolves TD-F036-09 (on-sale internal SEO detail page now exists).
    ...
    │
    ├── # Race Landing (F-083 cluster — lean-fork F-027, ZERO cross-module DI)
    ├── igloo-insurance/        # ⭐ FEATURE-085 NEW: tự phát sinh đơn bảo hiểm Igloo (giữ volume HĐ, KHÔNG bán). Load trong platformDbModules (inject DataSource('platform')). `utils/igloo-helpers.ts` pure (normalizePhone/isValidIdCard 9-12/computeCoverage 1-day/computePremium FLAT 10000/derivePackageCode→ROAD/isEligible/buildPartnerRefId/buildIglooPayload) + `schemas/igloo-insurance-request.schema.ts` (collection `igloo_insurance_requests`, unique partnerRefId=`igloo:<athletesId>:<raceId>` idempotency + payloadSnapshot frozen) + `services/`: igloo-http (X-API-Key) / igloo-selection (raw SQL parameterized JOIN athletes+athlete_subinfo+races, created_on-today + RAND() top-up) / igloo-request (Mongo + E11000 idempotency + state + retry) + controller 7 endpoint `@UseGuards(LogtoAdminGuard)` + `crons/`: igloo-daily (gated IGLOO_DAILY_ENABLED) / igloo-submit-worker (gated IGLOO_SUBMIT_ENABLED — chốt egress) / igloo-poll-worker. **2 kill-switch ENV default false → dev 0 egress.** Phí cố định 10k/ROAD/1-day (Danny chốt). KHÔNG mask PII. 55 test. Igloo API `api-igloo-insurance.5solution.vn` đứng trước GIC. Reuse `athletes.created_on` (added AthleteReadonly) + RaceReadonly (promo-hub).
    ├── landing/                # ⭐ FEATURE-083 NEW: per-race promo microsite (subdomain Phase 1). 1 schema (`race_landings` — unique `raceRef.raceId` + sparse-unique `domain.subdomain` + section subdoc array + `publish.liveSnapshot`) + 6 DTO files + `landing.constants.ts` (10 section type: hero/about/course/schedule/pricing/results_embed/photos_embed/gallery/sponsors/contact_social + VARIANTS_BY_TYPE + RESERVED_SUBDOMAINS + SUBDOMAIN_REGEX + LANDING_CACHE) + service + controller + spec (15 TC). 10 endpoints: `slug/:slug` + `resolve` (PUBLIC, route-order BEFORE `:id`) + 8 admin (`@UseGuards(LogtoAdminGuard)`: create-seed/list/findById/update/reorder/publish/unpublish/delete). **Lean-fork F-027 plumbing (NO import):** SETNX `landing-lock:<sub>` retry 3×200ms + cache raw-on-write/strip-on-read + sanitize-html. **Publish = atomic version-guarded `findOneAndUpdate({_id,'publish.version':cur})`** freeze `enabled` sections → `liveSnapshot`; public serves liveSnapshot ONLY. **Public strip BR-83-20 = allowlist literal** (`toPublicResponse` — `_id`→`id`, NO merchantRef/internalName/publish/status). Reads `Race` model (forFeature) for create-seed (no RacesService DI). Redis `landing:slug:`(60s stripped DTO)/`landing:resolve:`(300s)/`landing-lock:`(5s). S3 `landing-assets/` no-expire (rule 7).
    │
    ├── crew-certificates/      # ⭐ FEATURE-090 NEW: GCN cho Crew sự kiện. 2 schema (`crew_cert_batches` — unique slug + embed `CrewTemplate` reuse certificates sub-schema canvas/layer/photoArea, decouple race_id; `crew_cert_recipients` — index {batchId, normalizedName=slugifyVN}) + roster-parser (exceljs xlsx + papaparse csv, header VN normalize, photoUrl http/https only) + service + controller. Endpoints: admin CRUD batch + roster preview/confirm (2-step port F-032) + `:id/preview` (PNG mẫu) ; PUBLIC `public/:slug/search?name=` (anti-enum ≥2/≤20/no-list-all/no-leak) + `public/render/:recipientId` (PNG). **REUSE `CertificateRenderService`** (CertificatesModule exports) — engine mở rộng ADDITIVE generic variables ({full_name}/{position}/<cột>) + `RenderableTemplate`. Render data map runner_name=fullName, photoUrl, variables. Redis `crew-cert:render:`(600s base64)/`crew-cert-lock:`(5s). S3 phôi `crew-certificates/` persist (rule 8). Admin editor 3-tab (form toạ độ + live preview, KHÔNG Konva). Public `/gcn/<slug>` (frontend). 0 dep mới.
    │
    ├── short-links/            # ⭐ FEATURE-089 NEW: generic URL shortener (`s.5bib.com/<code>`). 1 schema (`short_links` — unique `code`, targetUrl, clickCount, active, createdBy) + 3 DTO + `short-links.constants.ts` (base62 crypto code-gen + reserved set case-insensitive + SHORTLINK_CACHE port LANDING_CACHE) + service + controller + 2 spec (15 service + 12 QC). Endpoints: `resolve/:code` (PUBLIC, route-order BEFORE `:id`, server-to-server từ FE) + admin CRUD + `:id/qr.png` (StreamableFile, `qrcode` lib). **Redirect served FRONTEND** (middleware `s.5bib.com/<code>`→`/r/<code>` route handler→resolve→302) vì backend `setGlobalPrefix('api')`. Cache-aside + SETNX `shortlink-lock:` (port landing, + re-read-cache-before-404 fix). `$inc` click fire-and-forget mọi resolve. Redis `shortlink:code:`(3600s string)/`shortlink-lock:`(5s). 0 dep mới, 0 platform-DB.
    │
    └── upload/                 # AWS S3 file uploads. ⚠ CRIT-03 ZERO validation (Danny defer 2026-05-12 — dev có lý do, có WAF/CloudFront cover). dùng `file.originalname` raw + ContentType from client header (XSS risk via S3 URL). ⭐ FEATURE-083: ADDED optional sanitized `folder` param to `uploadFile(file, folder?)` (strip `..`/unsafe chars/edge-slashes; backward-compat → date prefix when omitted).
```
...
│   ├── giai-chay/              # ⭐ FEATURE-036 NEW SEO subdirectory routes /giai-chay/[slug] + /[slug]/ket-qua. ⭐ FEATURE-037 V2 (2026-05-18): RaceCard reverted on-sale external link → all sources internal Link, RaceCTA BR-37-11 registration-closed disabled visual, dual-source detail page rendering (MongoDB precedence → MySQL on-sale fallback via getRaceBySlug rewrite in seo-api.ts)
│   ├── sitemap-races.xml/      # ⭐ FEATURE-036 NEW route.ts dynamic XML sitemap. ⭐ FEATURE-037 V2: now includes 17 on-sale slugs priority 0.9 (results URL still skip for on-sale — no /ket-qua page)
