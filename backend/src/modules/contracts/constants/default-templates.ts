 ...
 *
 * ─────────────────────────────────────────────────────────────────────────
 * QC ROUND 2 FIX LOG (2026-05-11) — 15 issue resolution
 * ─────────────────────────────────────────────────────────────────────────
 *
 * CRITICAL (2):
 *   1. TIMING Art 11: tách paragraph "auto-liquidate 7 ngày" + "02 bản gốc" thành 2 đoạn riêng
 *      (trước đây bị merge sai nghĩa pháp lý — mất cụm "được tự động thanh lý").
 *   2. TICKET_SALES Art 4: replace hardcoded `6.5%` → `{ticketFeePercent}%` placeholder
 *      để consistent với Art 1 line 546.
 *
 * HIGH (8):
 *   3. TIMING Art 8 reference "Điều 2": raw DOCX gốc ghi "Điều 5" (legal team typo —
 *      Điều 5 TIMING là CHẤM DỨT HĐ, không phải lãi suất). Coder silent fix về "Điều 2"
 *      (đúng — Điều 2 TIMING định nghĩa lãi suất {paymentTerms.latePenaltyRate}).
 *   4. SHARED Art 8 reference "Điều 6": raw DOCX RACEKIT/OPERATIONS ghi "Điều 5" (sai —
 *      trong RACEKIT/OPS, Điều 5 = QUYỀN LỢI BÊN B, Điều 6 mới là CHẤM DỨT HĐ).
 *      → TIMING ref "Điều 2" và SHARED ref "Điều 6" là CỐ Ý KHÁC NHAU vì numbering
 *      của TIMING (Đ5 = CHẤM DỨT) khác RACEKIT/OPS (Đ6 = CHẤM DỨT).
 *   5. TIMING Art 6: giữ literal "05 (năm) ngày" (legal standard cho insurance proof —
 *      không nên placeholderize vì BTC/đối tác kỳ vọng số ngày standardized).
 *   6. OPERATIONS Art 1 setup/expoday/raceday: giữ free-format string + comment lý do —
 *      Danny chốt B: admin nhập "06:00 ngày 15/05/2026 đến 12:00 ngày 16/05/2026"
 *      vào field {raceDate} cho race nhiều ngày, không thêm placeholder mới.
 *   7. TIMING Art 6 line ~292: typo quote "Bảo hiểm thiết bị" (raw thiếu close quote)
 *      → Coder silent fix thêm close quote. ACCEPT — đúng văn phong, document tại đây.
 *   8. TICKET_SALES Art 7: reword "Điều 1.7 và 1.8" → "Điều 1 (Phí bán vé và Phí quản lý
 *      vận động viên)" vì Điều 1 trong template KHÔNG có numbered sub-clause 1.7/1.8.
 *      Reword là CORRECT, document tại đây.
 *   9. TICKET_SALES Art 1: restore sub-heading "Thông tin về vé của sự kiện:" trước
 *      dòng "Loại vé: Vé điện tử" để khớp structure raw DOCX (2 sub-section khác nhau:
 *      thông tin sự kiện vs thông tin vé).
 *  10. RACEKIT/OPERATIONS Art 1 {raceDate}: free-format STRING — admin nhập tự do
 *      (vd: "Từ 08:00 ngày 01/05/2026 đến 19:30 ngày 02/05/2026" cho race nhiều ngày).
 *      KHÔNG thêm placeholder mới {raceStartDate}/{raceEndDate} (Danny chốt B).
 *
 * LOW (5) — silent typo fix vs file gốc (audit trail):
 *  11. TIMING Art 3: raw DOCX có heading "Thời điểm xác lập việc nhận thông báo" lặp
 *      2 lần (typo legal) → extracted sửa heading 2 = "Thay đổi thông tin liên hệ",
 *      heading 3 = "Đầu mối liên hệ".
 *  12. TIMING/RACEKIT/OPS Art 7: raw typo "tác động trở sẽ bị tạm ngừng thực hiện vụ"
 *      → extracted "Bên bị tác động sẽ bị tạm ngừng thực hiện".
 *  13. RACEKIT Art 5 raw: "ưMột trong hai bên..." (extra "ư" prefix) → fix "Một...".
 *  14. TICKET_SALES Art 5: DROP HẲN section "Đối soát email roster" (PII hardcoded:
 *      danny@5bib.com, ketoan@5bib.com, Nguyễn Bình Minh, Trần Thanh Bình). Danny chốt A:
 *      admin override per-contract qua templateOverrides.articles khi cần NĐ 13/2023 PII.
 *  15. TICKET_SALES Art 8: DROP HẲN section "Thông tin người tiếp nhận chuyển giao
 *      dữ liệu" (NĐ 13/2023/NĐ-CP PII contact). Danny chốt A: admin override khi cần.
 *
 * ─────────────────────────────────────────────────────────────────────────
 *
 ...
 *   {raceName}               — tên giải
 *   {raceDate}               — ngày diễn ra giải (FREE-FORMAT STRING — admin nhập tự do.
 *                              Race nhiều ngày: "Từ 08:00 ngày 01/05/2026 đến 19:30
 *                              ngày 02/05/2026". KHÔNG split start/end placeholder
 *                              riêng — Danny chốt B 2026-05-11.)
 ...
 *   {ticketFeePercent}       — % phí bán vé (TICKET_SALES) — DÙNG CHUNG cho Art 1 + Art 4
...
Để đảm bảo quyền lợi của Khách Hàng, Bên B được quyền thay mặt Bên A thực hiện nghĩa vụ hoàn tiền đối với Khách Hàng khi Bên A đồng ý xác nhận và Bên A cam kết ngoài Phí bán vé, Bên A sẽ thanh toán lại cho Bên B toàn bộ số tiền mà Bên B đã chi trả thực tế cho Khách Hàng (nếu có) khi Bên A cung cấp các hồ sơ chứng từ chứng minh các khoản chi đó là hợp pháp và được Bên A thống nhất trước khi thực hiện. Lưu ý rằng với các trường hợp hoàn tiền này, Bên B vẫn được thu Khoản phí bán vé tương đương {ticketFeePercent}% trên tổng số tiền mà khách hàng đã thanh toán mua vé của Bên A.
